...
**数据块行最大值索引（Tile Row ArgMax）**

`TROWARGMAX` 对输入 Tile 的每一行所有列求最大值的位置索引（argmax），结果写入输出 Tile 中。可选同时输出每行最大值。
...
for r in 0..(Rv-1):                              // 遍历所有行
  ...
  for c in 1..(Cv-1):                            // 遍历所有列
    if src[r, c] > max_val:
      max_val = src[r, c]
      max_idx = c
  dstIdx[r, 0] = max_idx                          // 输出最大值列索引
  // 可选：dstVal[r, 0] = max_val                 // 输出最大值
```
...
```asm
    TROWARGMAX <LB0:ValidCol, LB1:ValidRow, LB2:Col, DataType, PadValue>, SrcTile<.reuse>, ->DstIdx<Size>, DstVal<Size>
```
...
- **DstIdx**：索引输出 Tile 寄存器（`S32`/`U32` 类型），每行输出最大值对应的列索引，支持 `T`/`U`/`M`/`N` 队列输出。
- **DstVal**：最大值输出 Tile 寄存器（可选，可缺省），输出每行最大值，类型与 `DataType` 一致。
...
- [BSTART.TEPL](../../blockIntro/tepl_block/header.md) `TROWARGMAX, DataType`
...
```asm
    TROWARGMAX <LB0:32, LB1:16, FP32, zero>, T#1.reuse, ->T<64B>, U<64B>
```

1. **操作内容**
    - 输入：`T#1` Tile 寄存器（源数据）
    - 输出：索引结果存入 `T` 队列 Tile 寄存器；最大值结果存入 `U` 队列 Tile 寄存器
2. **数据处理范围**
    - 有效列数 `32`（由 `LB0:32` 指定）
    - 有效行数 `16`（由 `LB1:16` 指定）
3. **数据格式**
    ...
    - 索引输出为 `S32` 类型
4. **输出**
    - `T<64B>`：16 行 × 1 列 × 4 字节 = 64B 索引输出
    - `U<64B>`：16 行 × 1 列 × 4 字节 = 64B 最大值输出
