...
**数据块部分逐元素最大值（Tile Partial Element-wise Maximum）**

`TPARTMAX` 在目标 Tile 的有效区域内对两个源 Tile 执行逐元素最大值选择。两个源 Tile 的有效区域可能不完全重叠：重叠处取最大值，仅单侧有效处传播该侧的值，均无效处由实现定义。
...
```asm
    TPARTMAX <LB0:ValidCol, LB1:ValidRow, LB2:Col, DataType, PadValue>, SrcTile0<.reuse>, SrcTile1<.reuse>, ->DstTile<Size>
```
...
- [BSTART.TEPL](../../blockIntro/tepl_block/header.md) `TPARTMAX, DataType`
