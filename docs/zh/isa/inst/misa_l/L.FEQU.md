...
![L.FEQS](../../../figs/bitfield/svg/Instruction_64bit/L.FEQU.svg)
...
!!! note "注意！"
    ...
    本指令为**无序比较**（Unordered）：若任一操作数为NaN（包括SNaN或QNaN），结果为`1`（True），且不触发异常。

## 备注

1. 本指令属于[超长指令扩展](../../instset/longInstrs.md)，可用于向量数据块或访存数据块的块体内。
2. 本指令的向量版本请见[V.FEQU](../misa_v/V.FEQU.md)。
