...
整型向量点积16×2（*Vector Dot Product 16×2*）<br>
对SrcL和SrcR中同一个lane的两组16-bit整型值分别相乘（每个32-bit lane中包含高16-bit和低16-bit各一对），并将每2个连续lane的4个乘积累加起来，加上SrcD中最小lane的累加初值，结果广播（broadcast）到这2个lane中。

## 汇编语法

```asm
    v.dot.16x2 SrcL<.reuse>.{T}, SrcR<.reuse>.{T}, SrcD<.reuse>.{T}, ->RegDst.{W}
```
...
- **T**：指定操作数的数据类型，仅支持sw, uw（32-bit宽输入，每个lane内打包2个16-bit值）。
- **->**：用于指示目的寄存器。
- **RegDst**：目的寄存器。
...
for (gid = 0; gid < lanenum / 2; gid++) {
    ...
    for (j = 0; j < 2; j++) {
        ...
        if (pmask[laneid] == 1) {
            ...
            // 每个 32-bit lane 内打包 2 个 16-bit 值
...
点积运算中，16-bit中间乘法结果应扩展到32-bit后再累加，以避免溢出。
