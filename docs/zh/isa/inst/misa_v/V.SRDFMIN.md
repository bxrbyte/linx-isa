...
子组浮点最小值归约（*Sub-group Reduce Floating-point Minimum*）<br>
将当前Group内的lane划分为若干子组（sub-group），在每个子组内对源寄存器的浮点数值比较得到最小值，结果广播（broadcast）到该子组的所有lane中。子组大小由SrcR标量寄存器指定。

## 汇编语法

```asm
    v.srdfmin SrcL<.reuse>.{T}, SrcR<.reuse>, ->RegDst<.W>
```
...
for (sg_id = 0; sg_id < num_subgroups; sg_id++) {
    bits(64) minvalue = MAX_VALUE;   // 初始化为输入类型最大值
...
本指令为0.57版本新增的子组归约指令，旧版V.RDFMIN（无范围参数）仍被保留。
