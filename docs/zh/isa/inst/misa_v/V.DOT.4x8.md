...
整型向量点积4×8（*Vector Dot Product 4×8*）<br>
对SrcL和SrcR中同一个lane的8组4-bit整型值分为低4组和高4组，每组内4对值分别相乘后再累加，并分别加上SrcD中对应lane低16-bit和高16-bit的累加初值，两个结果分别写入目的寄存器的低16-bit和高16-bit。

## 汇编语法

```asm
    v.dot.4x8 SrcL<.reuse>.{T}, SrcR<.reuse>.{T}, SrcD<.reuse>.{T}, ->RegDst.{W}
```
...
- **T**：指定操作数的数据类型，仅支持sw, uw（32-bit宽输入，每个lane内打包8个4-bit值）。
- **->**：用于指示目的寄存器。
- **RegDst**：目的寄存器。
...
bits(64) pmask = P;
// 每个 lane 独立计算，2 组结果分别写入 Dst 低/高 16-bit
...
点积运算中，4-bit中间乘法结果应扩展到更高精度后再累加，以避免溢出。
