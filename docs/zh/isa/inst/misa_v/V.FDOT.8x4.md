...
浮点向量点积8×4（*Vector Floating-point Dot Product 8×4*）<br>
对SrcL和SrcR中同一个lane的4组8-bit浮点值分别相乘（每个32-bit lane内打包4个8-bit值），再将乘积累加起来，加上SrcD中对应lane的累加初值，结果写入目的寄存器。每个lane独立计算，不做跨lane归约。与V.DOT.8x4逻辑相同，但操作数为浮点类型。

## 汇编语法

```asm
    v.fdot.8x4 SrcL<.reuse>.{T}, SrcR<.reuse>.{T}, SrcD<.reuse>.{T}, ->RegDst.{W}
```
...
- **T**：指定操作数的数据类型，仅支持fs（32-bit宽输入，每个lane内打包4个8-bit浮点值）。
- **->**：用于指示目的寄存器。
- **RegDst**：目的寄存器。
