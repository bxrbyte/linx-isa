...
子组按位或归约（*Sub-group Reduce Bitwise OR*）<br>
将当前Group内的lane划分为若干子组（sub-group），在每个子组内对源寄存器的整数值执行按位或归约，结果广播（broadcast）到该子组的所有lane中。子组大小由SrcR标量寄存器指定。

## 汇编语法

```asm
    v.srdor SrcL<.reuse>.{T}, SrcR<.reuse>, ->RegDst<.W>
```
...
本指令为0.57版本新增的子组归约指令，旧版V.RDOR（无范围参数）仍被保留。
