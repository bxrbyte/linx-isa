...
- **W**：目的寄存器的位宽标识，必须与源类型位宽一致（b,h,w,d）。
- **rm（rounding mode）**：舍入模式的标记符。
- **sat（saturation）**：支持饱和计算的标志。

## 编码格式

![V.FRINT](../../../figs/bitfield/svg/Instruction_64bit/V.FRINT.svg)
...
- 本指令与 [V.FCVT](V.FCVT.md) 的区别在于：V.FRINT 保持浮点类型不变，仅对尾数进行舍入；V.FCVT 在不同浮点类型之间进行完整格式转换。
- 与 [V.FCVTI](V.FCVTI.md) 的区别在于：V.FRINT 结果为浮点型整数（如 4.0）；V.FCVTI 结果为真正的整型（如 4）。
