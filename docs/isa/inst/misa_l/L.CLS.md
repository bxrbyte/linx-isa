...
计数前导符号位(*Count Leading Sign bits*)<br>
从源操作数的 `M` 位开始，连续 `N` 位的范围内，从高位至低位计数与符号位相同的连续位数，将计数结果写到目的寄存器中。

因此，如果输入的最高有效位为0，则输出为前导零的个数；如果输入的最高有效位为1，则输出为前导一的个数。

## 汇编语法

```asm
    l.cls SrcL.<T>, M, N, ->RegDst.d
```
...
![L.CLS](../../../figs/bitfield/svg/Instruction_64bit/L.CLS.svg)
...
2. 本指令的向量版本请见[V.CLS](../misa_v/V.CLS.md)。
